...
class App extends Component {
    ...
    // Mint NFT là quá trình mã hóa một tác phẩm nghệ thuật hay tài sản thông thường trở thành một phần của blockchain
    mint = (kryptoBird) => {
    ...
    render() {
        return ( <
            ...
            <
            h1 style = {
                { color: 'black' } } > KryptoBirdz - NFT Marketplace < /h1> <
            form onSubmit = {
                (event) => {
                    event.preventDefault()
                    const kryptoBird = this.kryptoBirdz.value
            ...
            div className = "row textCenter" > {
                this.state.kryptoBirdz.map((kryptoBird, key) => {
                    return ( <
                        ...
                        MDBCardText > The KryptoBirdz are 20 uniquely generated KBirdz from the cyberpunk cloud galaxy Mystopia!There is only one of each bird and each bird can be own by a single person on the Ethereum blockchain. < /MDBCardText> <
